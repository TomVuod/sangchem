...
lm_model <- lmer(I((predicted_species))~sang_prop+(1|colony)+(1|colony:census_date), data=model_input)
print(summary(lm_model))
...
```
```{r fig.cap="Distribution of the differences in the performance of the discriminant models trained on true and permuted data. Age status of \\textit{F. sanguinea} ants were shuffled in the null variant.", ref.label='perf-test'}
...
hist(discrimination_model_validation[[1]]-discrimination_model_validation[[2]], breaks=15, xlab = "Difference in AUROC",
     main = "Basic vs. null model")
```

All differences are positive, so the *p*-value is less than 0.001 since the null distribution consists of 1000 values (Figure \@ref(fig:perf-test)).
...
$$
\log(\text{CHC_mass_callow+1}) =  \beta_0 + \text{sanguinea_proportion} + \newline(1|\text{colony_ID}) + (1|\text{colony_ID:sampling_occasion_ID}) + \epsilon,
$$
...
lm_model <- lmer(I(log(normalized_mass+1)) ~ sang_prop + (1|colony), data=model_input)
print(summary(lm_model))
...
library(dplyr)
library(sangchem)
separation_data$cocoon_1 <- as.numeric(separation_data$cocoon_1)
separation_data$cocoon_2 <- as.numeric(separation_data$cocoon_2)
set.seed(3810)
dist_matr <- sangchem:::global_distances_matrix()
n_rep = 1e3
test_stats_c <- list()
...
for(n in 1:4){
  ...
  for(i in seq_len(n_rep)){
    ...
    res <- wilcox.test(distance_df$d_related, distance_df$d_unrelated, paired = TRUE)
    test_stats_c[[n]]$V <- c(test_stats[[n]]$V, res$statistic)
    test_stats_c[[n]]$p_val <- c(test_stats[[n]]$p_val, res$p.value)
  }
...
These calculations do not provide the absolute value of body surface area. Their results were converted into scaling factors by using them as divisors of a standard area ($A_0\over\hat{A}$). In this way, the unit of measurement is canceled out, and absolute values are converted into dimensionless ratios. The standard area, $A_0$, was calculated by substituting the $w$ = 1.15 mm into the formula above and using coefficients obtained from measurements of *F. sanguinea*. 
