...
print(res)
peaks_callow <- res$chromatogram_ID[res$predicted_species<=quantile(res$predicted_species, 0.2)]
```
...
res <- res[order(res$predicted_species),]
print(res)
peaks_mature <- res$chromatogram_ID[res$predicted_species<=quantile(res$predicted_species, 0.2)]
```
...
We will verify the performance the above model against the null ones for which the age status of *F. sanguinea* ants is shuffled. 
