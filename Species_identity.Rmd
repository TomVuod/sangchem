...
plot(apply(PCA_callow_discr$x,2,sd))
n_PCs <- sum(cumsum(apply(PCA_callow_discr$x,2,sd))/sum(apply(PCA_callow_discr$x,2,sd))<0.9)+1
...
```
```{r fig.cap="Results of the performance test of the discriminant model with different number of components. For details see the documunetation of R mixOmics package."}
...
# final model
species_prediction_model <- plsda(PCA_species_discr$x, Y, ncomp=selected_n, scale=FALSE)
```
...
res1 <- calculate_SII(incremented_matrix, PLS_DA_model=species_prediction_model, PCA_res = PCA_species_discr)
res2 <- calculate_SII(ones_matrix, PLS_DA_model=species_prediction_model, PCA_res = PCA_species_discr)
res <- res1
...
```
```{r}
kableExtra::kbl(res, longtable=TRUE, booktabs = TRUE, caption = 'List of the peaks with the species identity score indicating their importance as a marker of \\textit{F. fusca} or \\textit{F. sanguinea} samples.',col.names = c("Peak ID", "Compound", "Marker score", "\\emph{sanguinea} marker", "*fusca* marker")) %>%
  kable_styling(latex_options = c("repeat_header"))
```
The objects `peaks_sang`, `peaks_fusca`, `species_prediction_model` and `PCA_species_discr` are stored in the package folder and can collectively be loaded into the global environment along with the other objects by running `load_globals()`.
