...
We also need to define a reponse variable to be explained by discrimination model.
```{r}
...
print(peaks_sang)
cat(sprintf("Number of sanguinea markers: %d", length(peaks_sang)))
print(peaks_fusca)
cat(sprintf("Number of fusca markers: %d", length(peaks_fusca)))
```

The objects `peaks_sang`, `peaks_fusca`, `species_prediction_model` and `PCA_species_discr` are stored in the package and can be collectively loaded into the global environment along with some other objects by calling `load_globals()`.
```{r eval=FALSE}
...
We might ask how a given peak compares to other peaks in terms of being characteristic more of one or the other species. Correspondingly, we will generate artificial profiles which are composed of only one peak to see how each peak contributes to the species prediction.
...
Ants in mixed colonies exchange their CHCs to produce a hybrid chemical identity. Here we develop the method to assign each sample a number indicating how much it is characteristic of one or the other species.
The main idea is to fit a sPLS_DA model to the training set (sampled from field colonies) and use it to predict the explanatory variable (species) in the test data (samples from experimental colonies). However, the sPLS_DA model run on the raw data will yield the classificator which used only few variables (peaks) while unused variables can also be informative in the species discrimination task. We want to use as much data as it is possible to make robust predictions, Therefore we will transform input data with principal component analysis. The correspondig code in encapsulated in the `claculate_SII` function.
