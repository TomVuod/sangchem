...
## Species markers
This  report shows how the peaks discriminative for *F. sanguinea* and *F. fusca* were determined. We will use the chemical data collected from the filed colonies in the earlier study [@WLODARCZYK201798]. In this way our method of markers selection is blind to the dataset to which  markers will be applied avoiding potential source of the bias in the analysis results.
```{r warning=FALSE}
suppressMessages({
  library(sangchem)
  library(stringi)
...
for(peak in colnames(transformed_m)){
  ...
  p_val <- c(p_val, shapiro.test(sample(residuals(model), 3000))$p.value)
}
sort(peak_coefficients)
```
```{r}
